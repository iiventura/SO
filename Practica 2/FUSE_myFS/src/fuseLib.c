...
static int my_unlink(const char *path){
	...
	int iNodeID;
	if((iNodeID = findFileByName(&myFileSystem, (char *) path + 1)) == -1)
		return -ENOENT;

	// Cambiamos el tamaño a cero
	if(resizeNode(iNodeID, 0) < 0) return -EIO;

	// Liberamos el archivo indicado por iNodeID
	myFileSystem.directory.files[iNodeID].freeFile = true;
	myFileSystem.directory.numFiles--;

	// Aumentamos los nodos libres
	myFileSystem.numFreeNodes++;
	myFileSystem.nodes[iNodeID]->freeNode = true;

	// Actualizamos el directorio y el nodo
	updateDirectory(&myFileSystem);
	updateNode(&myFileSystem, iNodeID, myFileSystem.nodes[iNodeID]);

	// Liberamos el iNodeID
	free(myFileSystem.nodes[iNodeID]);
	myFileSystem.nodes[iNodeID] = NULL;
...
static int my_read(const char *path, char *buf, size_t size, off_t offset, struct fuse_file_info *fi) {
   	char buffer[BLOCK_SIZE_BYTES];
    int bytesReaded;
	int totaByteslReaded = 0;
    NodeStruct *node = myFileSystem.nodes[fi->fh];

    fprintf(stderr, "--->>>my_read: path %s, size %zu, offset %jd, fh %"PRIu64"\n", path, size, (intmax_t)offset, fi->fh);

    if(offset >= (node->fileSize)) return 0;

    if((node->fileSize-offset) < size) bytesReaded = node->fileSize - offset;
    else bytesReaded = size;

    while(totaByteslReaded < bytesReaded) {
        int pos; 
		int block; 
		int offBlock;
        ...
        block = node->blocks[offset / BLOCK_SIZE_BYTES];
        offBlock = offset % BLOCK_SIZE_BYTES;

        if(readBlock(&myFileSystem, block, &buffer) == -1) {
            fprintf(stderr,"Error reading blocks in my_read\n");
            return -EIO;
        }

		pos = offBlock;
		while(pos < BLOCK_SIZE_BYTES && totaByteslReaded < bytesReaded) {
            buf[totaByteslReaded++] = buffer[pos];
			pos++;
        }
...
struct fuse_operations myFS_operations = {
		...
		.read		= my_read,                      //Read data of a file
		.unlink		= my_unlink,
};
